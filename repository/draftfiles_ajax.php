...
switch ($action) {
    ...
    case 'zip':
        ...
        if ($newfile = $zipper->archive_to_storage(array($filepath => $file), $usercontext->id, 'user', 'draft', $draftid, $parent_path, $zipfile, $USER->id)) {
            $return = new stdClass();
    ...
    case 'downloaddir':
        ...
        if ($newfile = $zipper->archive_to_storage(array('/' => $stored_file), $usercontext->id, 'user', 'draft', $newdraftitemid, '/', $filename, $USER->id)) {
            $return = new stdClass();

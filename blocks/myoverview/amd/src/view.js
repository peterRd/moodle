...
function(
    ...
    var getFilterValues = function(root) {
        ...
        filters.prevDisplay = root.attr('data-prev-display');
     ...
     */
    var getMyCourses = function(root, filters, limit, pageNumber) {

        // Return a new promise with the current courselist to prevent calling
        // the repository for the same data when all we need is another display.
        if (filters.display != filters.prevDisplay) {
            root.attr('data-prev-display', filters.display);
            var courselist = new $.Deferred();
            courselist.resolve(currentCourseList);
            return courselist.promise();
        }
    ...
    var setCourseFavouriteState = function(courseId, status) {
        ...
        return Repository.setFavouriteCourses({
            ...
            if (result.warnings.length == 0) {
                currentCourseList.courses.forEach(function(course, index) {
                    if (course.id == courseId) {
                        currentCourseList.courses[index].isfavourite = status;
                    }
                });
                return true;
            } else {

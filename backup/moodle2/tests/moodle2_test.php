...
class core_backup_moodle2_testcase extends advanced_testcase {
    ...
    public function test_restore_course_using_plan_defaults() {
        ...
        $rc = new restore_controller($backupid, $course2->id,
            backup::INTERACTIVE_NO, backup::MODE_GENERAL, $USER->id,
            backup::TARGET_EXISTING_ADDING, null, backup::RELEASESESSION_NO, ['useplandefaults' => true]);
        $this->assertTrue($rc->execute_precheck());
        $rc->execute_plan();
        $rc->destroy();
        ...
        // Restore it on top of course2 (should NOT duplicate the forum).
        $rc = new restore_controller($backupid, $course3->id,
            backup::INTERACTIVE_NO, backup::MODE_GENERAL, $USER->id,
            backup::TARGET_EXISTING_ADDING);
        $this->assertTrue($rc->execute_precheck());
        $rc->execute_plan();
        ...
        // Get the forums now on the old course.
        $modinfo = get_fast_modinfo($course3->id);
        $forums = $modinfo->get_instances_of('forum');

...
class secondary extends navigation_node {
    ...
    protected function load_module_navigation() {
        ...
        // Nodes that require a custom construct.
        $customstructure = ['rolecheck'];

        if ($mainnode) {
            $this->add(get_string('module', 'course'), $this->page->url);
            ...
            $existingkeys = $mainnode->get_children_key_list();
            $leftover = array_diff($existingkeys, $populatedkeys);
            foreach ($leftover as $key) {
                if (!in_array($key, $customstructure) && $leftovernode = $mainnode->get($key)) {

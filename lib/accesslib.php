 ...
 * @param cache_application|cache_store|cache_session $cache A cache object generated from a similar cache::make in another function
 *                                                              to help with performance.
 * @return bool $value The original parameter.
 */
function update_access_cache($userid, $key, $value, $cache = null) {
    if (!$cache) {
...
function can_access_course(stdClass $course, $user = null, $withcapability = '', $onlyactive = false) {
    ...
    if ($withcapability and !has_capability($withcapability, $coursecontext, $userid)) {
        return update_access_cache($userid, $course->id, false, $cache);
    }
    ...
    if ($userid == $USER->id) {
        if (!empty($USER->access['rsw'][$coursecontext->path])) {
            ...
            return update_access_cache($userid, $course->id, true, $cache);
        }
    }
    ...
    if ($userid != $USER->id) {
        ...
        return update_access_cache($userid, $course->id, $isenrolled, $cache);
    }

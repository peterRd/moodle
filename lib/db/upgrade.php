...
function xmldb_main_upgrade($oldversion) {
    ...
    if ($oldversion < 2020032000.01) {
        ...
        upgrade_main_savepoint(true, 2020032000.01);
    }

    if ($oldversion < 2020032000.02) {

        $table = new xmldb_table('course_modules');

        // Adding new fields to table course_module table.
        $field = new xmldb_field('completionpassgrade', XMLDB_TYPE_INTEGER, '1', null,
            XMLDB_NOTNULL, null, '0', 'deletioninprogress');
        // Conditionally launch create table for course_completion_defaults.
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }

        $field = new xmldb_field('completionusegrade', XMLDB_TYPE_INTEGER, '1', null,
            XMLDB_NOTNULL, null, '0', 'completionpassgrade');
        // Conditionally launch create table for course_completion_defaults.
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
        ...
        upgrade_main_savepoint(true, 2020032000.02);
    }
